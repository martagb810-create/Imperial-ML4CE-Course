...
'''
def Objective
def Acquisition - combining different acquisition functions:
...
# Group Submission
group_names = ["Eric Lun", "Marta Garcia Belza"]
cid_numbers = ["", "02048342"]
oral_assignement = [1] #double check this is the one for BO assignment


######
#Sobol searchspace generation - currently attempting
######
def sobol_searchspace(
    ...
    # Generate Sobol points, between 0 and 1
    ...
    # Assign celltype, 0,1,2
    if balance_celltypes:
        cat_idx = np.arange(n_samples) % n_cat
    else:
        cat_idx = np.minimum((U[:, 5] * n_cat).astype(int), n_cat - 1)
    celltype_col = [celltypes[i] for i in cat_idx] # map back to names
    #POINT TO IMPROVE
    # Why not leave cat_idx as integers and use in BO directly?
...
# From Antonio, GP from scratch given
class GP_model:
